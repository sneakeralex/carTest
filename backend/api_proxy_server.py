...
class ApiSigner:
    ...
    def sign_request(method: str, url: str, body: str, original_headers: Dict[str, str],
        ...
        print(f"[DEBUG] Canonical URL: {canonical_url}")
        print(f"[DEBUG] Text to sign: {text_to_sign.replace(chr(10), '#')}")
...
def proxy(path):
    ...
    try:
        ...
        if body:
            print(f"[DEBUG] Request body: {body[:500]}{'...' if len(body) > 500 else ''}")
        ...
        print(f"[DEBUG] Signature headers: {signature_headers}")

        # Merge headers
        request_headers = {**headers, **signature_headers}

        print(f"[DEBUG] Final request headers: {request_headers}")
        ...
        print(f"[DEBUG] Response body: {response_body[:500]}{'...' if len(response_body) > 500 else ''}")
...
if __name__ == '__main__':
    print("Starting API Proxy Server on http://localhost:8888")
    print(f"Proxying requests to: {TARGET_BASE_URL}")
